...
void showBlockScreen(long* millis) {
    byte minutes = (byte)(*millis / 60000);
    byte seconds = (byte)(*millis / 1000 - minutes * 60);
    lcd.clear();
    lcd.setCursor(3, 0);
    lcd.print("Time left:");
    lcd.setCursor(5, 1);
    ...
    for (byte i = 0; i < strlen(timeLeft); i++) {
...
void loop() {
    ...
    do {
        ...
        leftMillis = rateLimiterCheckAndUpdateAttempts(&currentMillis);
        rateLimiterShiftAttempts(&currentMillis);
        if (leftMillis > 0) {
            showBlockScreen(&leftMillis);
        }
    } while (leftMillis > 0);
